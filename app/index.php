<!doctype html>
<html class="no-js">
<head>
    <?php include 'common/head.php';?>
</head>
<body>
    <?php include 'common/menu.php';?>
    <?php include 'common/header.php';?>
    <div id="ad-section1">
        <div class="container">
            <div class="col-md-3 text-center">
                <i class="fa fa-lightbulb-o fa-5x"></i>
                <h5 class="text-center">Residential Services</h5>
            </div>
            <div class="col-md-3 text-center">
                <i class="fa fa-users fa-5x"></i>
                <h5 class="text-center">My GA Power Account</h5>
            </div>
            <div class="col-md-3 text-center">
                <i class="fa fa-bolt fa-5x"></i>
                <h5 class="text-center">Commercial Utilities</h5>
            </div>
            <div class="col-md-3 text-center">
                <i class="fa fa-camera-retro fa-5x"></i>
                <h5 class="text-center">Business Services</h5>
            </div>
        </div>
    </div>
    <div id="ad-section2">
        <div class="container">
            <div class="col-md-4 text-center">
                <img src="assets/img/alabama-power-logo.png" alt="Alabama Power Logo" class="img-responsive" />
            </div>
            <div class="col-md-4 text-center">
                <img src="assets/img/gapower-southern-logo.png" alt="GA Power Southern Company" class="img-responsive" />

            </div>
            <div class="col-md-4 text-center">
                <img src="assets/img/southern-company-logo.gif" alt="Southern Company Logo" class="img-responsive" />
            </div>
        </div>
    </div>
    <?php include 'common/footer.php';?>
    <?php include 'common/foot.php';?>

</body>
</html>
